...
配置文件故意放在包外面，方便教学和实验时修改。推荐使用 YAML，因为它可读、
能写注释，也能把 bands 和 needlets 放在同一个文件里。

- `configs/default.yaml`：基础 NILC 教程配置。
- `configs/beam.yaml`：考虑 beam 的 NILC 教程配置。
...
但 beam 教程真正使用的 `lmax_alm` 仍然来自 `configs/beam.yaml`，不会被这个估计值偷偷覆盖。

## 教程脚本

这些脚本是教程，不是 pytest 测试：

- `tutorials/tutorial_nilc.py`：基础 NILC，使用 `configs/default.yaml`。
- `tutorials/tutorial_nilc_beam.py`：考虑 beam 的 NILC，使用 `configs/beam.yaml`。
